# Rerunning Cutadapt ------------------------------------------------------
# due to incomplete removal of primer sequences, I will rerun cutadapt
# I need to remove sample 052-LB1_S14_L001_R1 and _R2 due to insufficient file size.
...
# create output folder for cutadapt rerun using first cutadapt output as input:
path.2.cut2 <- here("data", "2.cut2")
if(!dir.exists(path.2.cut2)) dir.create(path.2.cut2)
names.2.cut2.R1 <- file.path(path.2.cut2, basename(names.1.data.R1))
names.2.cut2.R2 <- file.path(path.2.cut2, basename(names.1.data.R2))
...
inR1  <- win_to_wsl(names.2.cut.R1)
inR2  <- win_to_wsl(names.2.cut.R2)
outR1 <- win_to_wsl(names.2.cut2.R1)
outR2 <- win_to_wsl(names.2.cut2.R2)

# Optional: where to store per-sample logs (on Windows side, no conversion needed)
log_dir <- here::here("logs")
if (!dir.exists(log_dir)) dir.create(log_dir, recursive = TRUE)

# Rerun cutadapt on cutadapt output  --------------------------------------
# due to incomplete removal of primer sequences

# set cutadapt rerun input as previous cutadapt output  -------------------
...
for (i in seq_along(inR1)) {
  ...
  stderr_txt <- file.path(log_dir, sprintf("cutadapt_%s.stderr.txt", tools::file_path_sans_ext(basename(names.2.cut2.R1[i]))))
  ...
  if (res$status != 0L || !file.exists(names.2.cut2.R1[i]) || !file.exists(names.2.cut2.R2[i])) {
    stop(sprintf("Cutadapt failed for %s; see %s", basename(inR1[i]), stderr_txt))
  }
  
  # Quick success indicator (file sizes)
  s1 <- file.info(names.2.cut2.R1[i])$size
  s2 <- file.info(names.2.cut2.R2[i])$size
...
sapply(c(f, r.rc), primerHits, names = names.2.cut2.R1)
# GTCATCGGCCACGTCGACTCTGG GGAAGCCGCYGAGCTCGGHAAGG 
#                       0                       0 

sapply(c(r, f.rc), primerHits, names = names.2.cut2.R2)
# CCTTDCCGAGCTCRGCGGCTTCC CCAGAGTCGACGTGGCCGATGAC 
#                       0                       0 
