...
#' # set file path
path.6.phyloseq <- file.path(here("data", "6.phyloseq_on_merged_oomycetes_ITS_trimmed"))
...
ITS.ASVs <- readDNAStringSet(file.path(here("data","5.ITSx_merged_reads", "ITSx.oomycete.fasta"))) #Amplicon Sequence Variants (ITS.ASVs)
...
rownames(seqtab.ITS.ASVs) == rownames(taxtab)
...
colnames(seqtab.ITS.ASVs) == rownames(metadata)
